...
<script>
    ...
    let isMobileMode = false; // 기본은 PC 모드
    let activeMenuCell = null; // 모바일 팝업 메뉴가 떠있는 셀
    ...
    async function saveScore(autoName = null) {
        ...
        // 🚨 핵심 포인트: 모바일 모드면 난이도 뒤에 '_mobile'을 붙여서 아예 다른 데이터로 저장되게 만듦
    ...
    function renderRankingTable(diffId) {
        ...
        uniqueRanks.sort((a, b) => a.time - b.time); // 시간순 오름차순 정렬
    ...
    function handleInteraction(e, r, c) {
        ...
        // PC 모드일 때
        if (!isMobileMode) {
            if (e.buttons === 3) { handleChording(r, c); return; } // 양쪽 클릭 (혹시 몰라 남겨둠)
        ...
        if (e.button === 0) {
            let cell = board[r][c];
            if (cell.isRevealed) return; // 열린 칸은 무시 (더블클릭은 따로 처리됨)
